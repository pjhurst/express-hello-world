...
function getProductDetails(productNode) {
	return {
		...
		amount: productNode.querySelector(".price").innerHTML.replace("$", ""),
	};
}
...
function applePayShippingContactSelected(session, request, event) {
	...
	var updatedRequest = selectApplePayShippingMethod(
		request,
		shippingMethods[0]
	);
...
function startApplePay() {
	...
	Array.prototype.forEach.call(applePayButtons, function (button) {
		...
		button.addEventListener("click", function (e) {
			...
			e.preventDefault();
			var request = createApplePayPaymentRequest(
				getProductDetails(e.target.parentNode.parentNode)
			);
		...
		},
		{
			...
			data: {
				...
				paymentMethodTokenizationParameters: {
					...
					parameters: {
						publicKey:
							"BO39Rh43UGXMQy5PAWWe7UGWd2a9YRjNLPEEVe+zWIbdIgALcDcnYCuHbmrrzl7h8FZjl6RCzoi5/cDrqXNRVSo=",
					},
				},
			},
		},
	...
	return Object.assign({}, details, {
		...
		total: {
			...
			amount: {
				currency: currencyCode,
				value: (
					parseFloat(details.total.amount.value) +
					parseFloat(selectedShippingMethod.amount)
				).toString(),
			},
		},
	});
...
function shippingAddressChange(request, details, event) {
	...
	event.updateWith(
		Promise.resolve(
			selectShippingOption(details, shippingMethods[0].identifier)
		)
	);
}
	...
	Array.prototype.forEach.call(buyNowButtons, function (button) {
		button.addEventListener("click", function (e) {
			...
			var request = new PaymentRequest(
				requestData.methodData,
				requestData.details,
				requestData.options
			);
			request.addEventListener(
				"shippingaddresschange",
				shippingAddressChange.bind(window, request, requestData.details)
			);
			request.addEventListener(
				"shippingoptionchange",
				shippingOptionChange.bind(window, request, requestData.details)
			);
			request.show().then(
				function (instrument) {
					console.log(instrument);
					instrument.complete().then(function () {
						window.location = "order-confirmation.html";
					});
				},
				function (failure) {
